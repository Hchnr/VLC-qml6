...
static vlc_tick_t main_system_to_stream(vlc_clock_main_t * main_clock,
                                       vlc_tick_t system)
{
    if (unlikely(main_clock->offset == VLC_TICK_INVALID))
                                       ...
                                       vlc_tick_t pts, float rate)
{
    ...
    if (main_clock->reset_date != VLC_TICK_INVALID)
    {
        ...
        main_clock->reset_date = VLC_TICK_INVALID;
    ...
    if (pts != VLC_TICK_INVALID && system_now != VLC_TICK_INVALID)
...
static void vlc_clock_slave_reset(vlc_clock_t * clock)
{
    vlc_clock_main_t * main_clock = clock->owner;
    vlc_mutex_lock(&main_clock->lock);
    main_clock->wait_sync_ref = clock_point_Create(VLC_TICK_INVALID, VLC_TICK_INVALID);
    vlc_mutex_unlock(&main_clock->lock);
}
